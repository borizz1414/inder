...
export class ListCorrectiveMaintenanceContainer implements OnInit {
  ...
  configCard = {
    ...
    routerLinkBtn: "/mantenimientos-correctivos/coordinación/agregar",
  };
  ...
  columnsNamesToDisplay = [
    ...
    { name: "tipo_mantenimiento", display: "TIPO DE MANTENIMIENTO" },
    { name: "fecha", display: "FECHA" },
    { name: "estado_correctivo", display: "ESTADO" },
  ...
  configDisplayElement = {
    ...
    actions: [
      {
        action: "view",
        actionsLink: "/mantenimientos-correctivos/coordinación/detalle",
      },
    ],
  };
    ...
    this._maintenance.getAllMaintenances(params, this.endpointParam).subscribe(
      (resp: any) => {
        ...
        this.dataSource = resp.data.length ? resp.data.map(element => ({
          ...
          tipo_mantenimiento: element?.tipo_mantenimiento?.nombre,
          fecha: element?.updated_at ? moment(element.updated_at).format("YYYY-MM-DD") : '',
          estado_correctivo: element?.estado?.nombre,
        })) : []
